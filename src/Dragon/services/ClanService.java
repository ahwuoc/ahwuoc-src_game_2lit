...
import static Dragon.models.clan.Clan.DEPUTY;
import static Dragon.models.clan.Clan.LEADER;
import static Dragon.models.clan.Clan.MEMBER;
import Dragon.models.item.Item;
import Dragon.consts.ConstNpc;
import Dragon.models.Template.FlagBag;
import Dragon.models.clan.Clan;
import Dragon.models.clan.ClanMember;
import Dragon.models.clan.ClanMessage;
...
//                    askForPea(player);
                    ...
                    if (!isMeInClan) {
                        if (clan.getCurrMembers() < clan.maxMember) {
                            ...
                            if (!asked) {
                                ...
                                cmg.playerId = (int) player.id;
                                cmg.playerName = player.name;
                                cmg.playerPower = (long) player.nPoint.power;
                                cmg.role = -1;
                                clan.addClanMessage(cmg);
                                clan.sendMessageClan(cmg);
                            }
    ...
    private void createClan(Player player, byte imgId, String name) {
        if (player.clan == null) {
            if (name.length() > 30) {
                Service.gI().sendThongBao(player, "Tên Bang Hội Không Được Quá 30 Kí Tự");
                return;
            }
